...
class PhotoMonthsModule(private val reactContext: ReactApplicationContext) :
    ...
    fun fetchMonths(promise: Promise) {
        try {
            ...
            cursor?.use {
                val keyIndex = it.getColumnIndex("monthKey")
                val countIndex = it.getColumnIndex("photoCount")
                ...
                while (it.moveToNext()) {
                    ...
                    if (parts.size == 2) {
                        try {
                            ...
                            val monthName = Month.of(month).getDisplayName(TextStyle.FULL, Locale.US) + " " + year
